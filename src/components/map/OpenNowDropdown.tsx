...
export function OpenNowDropdown({ value, onChange }: Props) {
  ...
  return (
    <div ref={ref} className="relative flex-shrink-0">
      ...
      {open && (
        <div className="absolute top-10 left-0 z-[500] rounded-2xl overflow-hidden py-1 min-w-[160px] bg-white border border-[#e5e7eb] shadow-[0_12px_32px_rgba(47,95,196,0.14)]">
          {OPTIONS.map(opt => (
            <button
              type="button"
              key={opt.value}
